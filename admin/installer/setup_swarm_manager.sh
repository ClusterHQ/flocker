...
docker pull swarm:1.2.1
docker run --rm -v ${DOCKER_CERT_HOME}:${DOCKER_CERT_HOME} swarm:1.2.1 create > /tmp/swarm-config/swarm_cluster_id
...
docker run -d -v ${DOCKER_CERT_HOME}:${DOCKER_CERT_HOME} -p 2376:2375 swarm:1.2.1 manage --tlsverify --tlscacert=${DOCKER_CERT_HOME}/ca.pem --tlskey=${DOCKER_CERT_HOME}/key.pem --tlscert=${DOCKER_CERT_HOME}/cert.pem token://$swarm_cluster_id
