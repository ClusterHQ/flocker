...
docker run -d -v ${DOCKER_CERT_HOME}:${DOCKER_CERT_HOME} -p 2376:2375 swarm manage --tlsverify --tlscacert=${DOCKER_CERT_HOME}/ca.pem --tlskey=${DOCKER_CERT_HOME}/swarm-manager-key.pem --tlscert=${DOCKER_CERT_HOME}/swarm-manager-cert.pem token://$swarm_cluster_id
