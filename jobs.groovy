...
/* jobs.groovy.j2
  ...
  jinja2 is only used to populate the config, which is loaded from build.yaml.
...
// the project name from the build yaml 'ClusterHQ/Flocker'
def project = GLOBAL_CONFIG.project
// the github https url for the project
def git_url = GLOBAL_CONFIG.git_url
branches = []
...
/* Add a particular job type to the multi-job. */
def add_to_multi_job(dashBranchname, job_type, job_type_values) {
    /* add the 'run_trial' style jobs, where there is a job per module */
    if (job_type in ['run_trial', 'run_trial_for_storage_driver',
                     'run_acceptance', 'run_client']) {
        for (job in job_type_values) {
            for (_module in job.value.with_modules) {
                _job_name = job.key + '_' + _module
                job("${dashProject}/${dashBranchName}/${_job_name}")  {
    ...
    if (job_type in ['run_sphinx', 'omnibus', 'run_lint']) {
        for (job  in job_type_values) {
              job("${dashProject}/${dashBranchName}/${job.key}")  {
                  /* make sure we don't kill the parent multijob when we
                     fail */
                  killPhaseCondition("NEVER")
              }
...
def build_multijob(dashBranchName, branchName) {
    ...
    multiJob("${dashProject}/${dashBranchName}/__main_multijob") {
        ...
        */
        triggers build_trigers('githubPush', 'none', branchName)
        wrappers {
            timestamps()
            colorizeOutput()
            ...
            phase('parallel_tests') {
                ...
                */
                for (job_type_entry in GLOBAL_CONFIG.job_type) {
                  add_to_multi_job(dashBranchname, job_type_entry.key,
                                   job_type_entry.value)
                }
            } /* ends parallel phase */
...
for (job_type_entry in GLOBAL_CONFIG.job_type) {
    job_type = job_type_entry.key
    for (job in job_type_entry.value) {
        job_values = job.value
        /* apply config related to 'cronly_jobs' jobs */
        if (job_type == 'cronly_jobs') {
            job("${dashProject}/${dashBranchName}/_{job.key}") {
                parameters {
                    textParam("TRIGGERED_BRANCH", "${branchName}",
                              "Branch that triggered this job" )
                }
